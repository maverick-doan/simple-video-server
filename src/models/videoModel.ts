...
export async function createVideo(params: {
    ...
    const q = `
        INSERT INTO s109.videos (id, owner_id, original_file_name, title, description, url, quality, duration_seconds, size_bytes)
...
export async function getVideoById(id: string): Promise<Video | undefined> {
    const q = `SELECT * FROM s109.videos WHERE id=$1 AND is_deleted=FALSE`;
