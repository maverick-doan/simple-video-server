-- Migration: 002__create__video_schema.sql

-- UP Migration
BEGIN;

CREATE TYPE video_quality AS ENUM ('1080p', '720p', '480p', '360p', '240p');
CREATE TYPE job_status AS ENUM ('pending', 'processing', 'completed', 'failed');

CREATE TABLE IF NOT EXISTS videos (
    id UUID PRIMARY KEY,
    owner_id UUID NOT NULL REFERENCES users(id),
    original_file_name TEXT NOT NULL,
    title TEXT NOT NULL,
    description TEXT,
    url TEXT NOT NULL,
    quality video_quality NOT NULL,
    duration_seconds INTEGER NOT NULL,
    size_bytes BIGINT NOT NULL,
    is_deleted BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT get_brisbane_timestamp(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT get_brisbane_timestamp()
);

CREATE TABLE IF NOT EXISTS transcode_jobs (
    id UUID PRIMARY KEY,
    video_id UUID NOT NULL REFERENCES videos(id),
    status job_status NOT NULL DEFAULT 'pending',
    requested_qualities video_quality[] NOT NULL,
    output_message TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT get_brisbane_timestamp(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT get_brisbane_timestamp()
);

CREATE INDEX IF NOT EXISTS idx_videos_owner ON videos(owner_id);
CREATE INDEX IF NOT EXISTS idx_videos_created_at ON videos(created_at);
CREATE INDEX IF NOT EXISTS idx_transcode_jobs_video ON transcode_jobs(video_id);
CREATE INDEX IF NOT EXISTS idx_transcode_jobs_status ON transcode_jobs(status);

CREATE OR REPLACE TRIGGER update_videos_updated_at 
    BEFORE UPDATE ON videos
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE OR REPLACE TRIGGER update_transcode_jobs_updated_at 
    BEFORE UPDATE ON transcode_jobs
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

COMMIT;

-- DOWN Migration
-- BEGIN;
-- DROP TABLE IF EXISTS videos;
-- DROP TABLE IF EXISTS transcode_jobs;
-- DROP INDEX IF EXISTS idx_videos_owner;
-- DROP INDEX IF EXISTS idx_videos_created_at;
-- DROP INDEX IF EXISTS idx_transcode_jobs_video;
-- DROP INDEX IF EXISTS idx_transcode_jobs_status;
-- DROP TRIGGER IF EXISTS update_videos_updated_at;
-- DROP TRIGGER IF EXISTS update_transcode_jobs_updated_at;
-- COMMIT;